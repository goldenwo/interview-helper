			...
			<div className="main-container" style={styles.container}>
				<header style={styles.header}>
					...
					<div style={styles.badges}>
						{jobDescription && (
							<span style={styles.badge}>{"\uD83D\uDCC4"} JD active</span>
						)}
						{resume && (
							<span style={styles.badge}>{"\uD83D\uDCCB"} Resume active</span>
						)}
					</div>
				</header>

				<main ref={mainRef} style={styles.main} onScroll={handleScroll}>
					<AnswerDisplay
						messages={messages}
				...
				<div style={styles.statusIndicator}>
					...
					<span style={styles.costText}>
						{hasPricing(settings.model)
							? `$${cost === 0 ? "0.00" : parseFloat(cost.toFixed(8))}`
							: "$?"}
					</span>
				</div>
