...
- Once the first real token arrives from the server, the stall phrase fades out
...
1. **Larger text:** Increase answer text size to ~18px (from current default) for readability at arm's length
...
```
Format your response as 3-5 concise bullet points. Each bullet should be
a short phrase or single sentence — not a paragraph. The user will
paraphrase these, so keep them scannable.
```

**Implementation location:** System prompt change in `server/index.ts`. Text size change in answer display CSS. `maxTokens` constant in server config.
...
```typescript
{
  ...
  resume: ResumeData | null,     // active resume context
  provider: string,
  model: string,
...
- If present and `timestamp` is within the last 30 minutes:
...
- One tap re-sends the same question with full message history, JD, and resume context
...
- Maintain a pricing table in `src/config.ts` for each supported model:
  ...
  ```
- Estimate tokens from character count: `tokens ≈ chars / 4`
...
- New endpoint: `GET /api/health` — returns 200 with empty body, no auth required
...
| Area | Files |
|------|-------|
| Stall phrases | New: `src/utils/stallPhrases.ts`. Modified: `App.tsx`, answer display CSS |
| Answer display | `server/index.ts` (system prompt, maxTokens), answer display CSS |
| Crash-proof state | `App.tsx` (in-flight persistence, recovery), `src/hooks/useChats.ts` (JD in chat) |
| Token budget | New: `src/hooks/useBudget.ts`. Modified: `src/config.ts` (pricing), `App.tsx`, `server/index.ts` (server cap) |
| Connection health | New: `src/hooks/useHealth.ts`. Modified: `server/index.ts` (/api/health), `src/api/getAnswer.ts` (timeout detection), `App.tsx` |
